...
$(document).ready(() => {
	...
	let amenities = {}, states = {}, cities = {};
	let check = {
    'amenities':{}
  };
	$(document).on('change', '.amenitiesCheck > li > input[type="checkbox"]', function () {
		if (!this.checked) {
      delete amenities[$(this).data('id')];
    }
		else amenities[$(this).data('id')] = $(this).data('name')

		let obj = Object.values(amenities)
  ...
  $(document).on('change', ".locations > .popover > li > input[type='checkbox']", function () {
		...
		let obj = Object.values(amenities)
		if (obj.length < 0) $('div.locations > h4')
